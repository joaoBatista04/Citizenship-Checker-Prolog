...
tem_direito(X, portugal) :- ((pai(Z, X); mae(Z, X); neto(X, Z)), cidadania(Z, portuguesa)), \+ cidadania(X, portuguesa).

% Verifica se X possui direito à cidadania britânica (transmitida por pais britânicos).
tem_direito(X, reino_unido) :- ((pai(Z, X); mae(Z, X)), cidadania(Z, britanica)), \+ cidadania(X, britanica).

% Verifica se X possui direito à cidadania alemã (transmitida até netos de alemães).
tem_direito(X, alemanha) :- ((pai(Z, X); mae(Z, X); neto(X, Z)), cidadania(Z, alema)), \+ cidadania(X, alema).